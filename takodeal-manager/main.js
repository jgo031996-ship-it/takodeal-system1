...
window.loadAppEditorDrafts = async function() {
  try {
    const customerSnap = await getDoc(doc(db, "settings", "customer_app_draft"));
    const posSnap = await getDoc(doc(db, "settings", "pos_app_draft"));

    const customer = customerSnap.exists() ? customerSnap.data() : {};
    const pos = posSnap.exists() ? posSnap.data() : {};

    document.getElementById("appEditCustomerAnnouncement").value = customer.announcement || "";
    document.getElementById("appEditCustomerBanner").value = customer.bannerImage || "";
    document.getElementById("appEditGcashName").value = customer.gcashAccountName || "JOSTUART O.";
    document.getElementById("appEditGcashNumber").value = customer.gcashNumber || "09629721305";
    document.getElementById("appEditGcashQr").value = customer.gcashQrImage || "QR.jpg";
    document.getElementById("appEditHiddenCategories").value = appEditorArrayToCsv(customer.hiddenCategories || ["Consumables", "Prep Batch", "Raw Materials", "Packaging"]);
    document.getElementById("appEditDeliveryEnabled").checked = customer.deliveryEnabled !== false;
    document.getElementById("appEditDeliveryRadius").value = customer.deliveryRadiusKm || 20;

    document.getElementById("appEditPosPayments").value = appEditorArrayToCsv(pos.paymentMethods || ["Cash", "GCash", "Bank", "Grab"]);
    document.getElementById("appEditPosOrderTypes").value = appEditorArrayToCsv(pos.orderTypes || ["Dine-In", "Take-Out", "Delivery", "Grab"]);
    document.getElementById("appEditPosTabs").value = appEditorArrayToCsv(pos.posTabs || ["Takoyaki", "Milk Tea", "Coffee", "Add-ons"]);
    document.getElementById("appEditKitchenPrep").value = appEditorArrayToCsv(pos.kitchenPrepCats || ["Prepared Batch"]);
    document.getElementById("appEditAuditItems").value = appEditorArrayToCsv(pos.auditItems || []);

    setAppEditorStatus("Draft settings loaded.");
  } catch (error) {
    console.error("App Editor load error:", error);
    setAppEditorStatus("Failed to load drafts. Check console.", true);
  }
};

window.saveCustomerAppDraft = async function() {
  const payload = {
    announcement: document.getElementById("appEditCustomerAnnouncement").value.trim(),
    bannerImage: document.getElementById("appEditCustomerBanner").value.trim(),
    gcashAccountName: document.getElementById("appEditGcashName").value.trim(),
    gcashNumber: document.getElementById("appEditGcashNumber").value.trim(),
    gcashQrImage: document.getElementById("appEditGcashQr").value.trim(),
    hiddenCategories: appEditorCsvToArray(document.getElementById("appEditHiddenCategories").value),
    deliveryEnabled: document.getElementById("appEditDeliveryEnabled").checked,
    deliveryRadiusKm: parseFloat(document.getElementById("appEditDeliveryRadius").value) || 20,
    updatedAt: serverTimestamp(),
    updatedBy: window.sessionUser ? window.sessionUser.email : "Manager"
  };

  await setDoc(doc(db, "settings", "customer_app_draft"), payload, { merge: true });
  setAppEditorStatus("Customer app draft saved. Live apps are untouched.");
};

window.publishCustomerAppDraft = async function() {
  if (!confirm("Publish Customer App v2 config? Current live customer app will not read this yet.")) return;
  const draftSnap = await getDoc(doc(db, "settings", "customer_app_draft"));
  if (!draftSnap.exists()) return alert("No customer draft found.");

  await setDoc(doc(db, "settings", "customer_app_live_v2"), {
    ...draftSnap.data(),
    publishedAt: serverTimestamp(),
    publishedBy: window.sessionUser ? window.sessionUser.email : "Manager"
  }, { merge: true });

  setAppEditorStatus("Customer app v2 config published. Not connected to live app yet.");
};

window.savePosAppDraft = async function() {
  const payload = {
    paymentMethods: appEditorCsvToArray(document.getElementById("appEditPosPayments").value),
    orderTypes: appEditorCsvToArray(document.getElementById("appEditPosOrderTypes").value),
    posTabs: appEditorCsvToArray(document.getElementById("appEditPosTabs").value),
    kitchenPrepCats: appEditorCsvToArray(document.getElementById("appEditKitchenPrep").value),
    auditItems: appEditorCsvToArray(document.getElementById("appEditAuditItems").value),
    updatedAt: serverTimestamp(),
    updatedBy: window.sessionUser ? window.sessionUser.email : "Manager"
  };

  await setDoc(doc(db, "settings", "pos_app_draft"), payload, { merge: true });
  setAppEditorStatus("POS app draft saved. Cashier app is untouched.");
};

window.publishPosAppDraft = async function() {
  if (!confirm("Publish POS App v2 config? This will not overwrite current global_pos_config.")) return;
  const draftSnap = await getDoc(doc(db, "settings", "pos_app_draft"));
  if (!draftSnap.exists()) return alert("No POS draft found.");

  await setDoc(doc(db, "settings", "pos_app_live_v2"), {
    ...draftSnap.data(),
    publishedAt: serverTimestamp(),
    publishedBy: window.sessionUser ? window.sessionUser.email : "Manager"
  }, { merge: true });
